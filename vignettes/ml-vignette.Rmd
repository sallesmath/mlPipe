...
### Package datasets
Inspect datasets to use mlPipe functions. In this tutorial we're going to use datasets present in mlbench and caret packages.

If you want to use other datasets, it is important that you organize your data in a dataframe (samples in the rows and characteristics of interest in the columns). Also, for the package to work properly, the response variable (or dependent variable) MUST be represented in the last column of your dataframe.
```{r}
...
Now that you have a list of trained machine learning models, it's useful to visualize them. In order to do so, you can use our ml.results.graphic function
```{r}
# Train your data with specific parameters. In this case, we're going to use:
# 80%/20% split between training and test set
# Machine learning algorithms: Random Forest (rf), Linear discriminant analysis (lda), Generalized Linear Model (glm), K-Nearest Neighbour (knn)
